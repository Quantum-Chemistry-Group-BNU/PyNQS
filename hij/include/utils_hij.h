...
void get_comb_2d(unsigned long *bra, unsigned long *comb, int n, int len,
  ...
  get_olst_cpu(bra, olst.get(), len);
  get_vlst_cpu(bra, vlst.get(), n, len);
...
torch::Tensor get_comb_tensor_cpu(torch::Tensor &bra_tensor, const int sorb,
  ...
  if (ms_equal) {
    ...
    // std::cout << "nsingles: " << nsingles << std::endl;
    // this is error for radical e.g. H3 H5 ...
    int ndoubles = noa * (noa - 1) * nva * (nva - 1) / 4 +
                   nob * (nob - 1) * nvb * (nvb - 1) / 4 +
                   noa * nva * nob * nvb;
    // std::cout << "ndoubles: " << ndoubles << std::endl;
    ncomb = 1 + nsingles + ndoubles;
  } else {
...
auto get_olst_vlst_cpu(torch::Tensor &bra_tensor, const int sorb,
                       const int nele) {
  const int no = nele;
  const int nv = sorb - nele;
  // const int tensor_len =(sorb-1)/8 + 1;
...
auto spin_flip_rand(torch::Tensor &bra_tensor, const int sorb, const int nele,
                    const int seed) {
  const int no = nele;
  const int nv = sorb - nele;
  const int bra_len = (sorb - 1) / 64 + 1;
  ...
  get_olst_cpu(bra_ptr, olst.get(), bra_len);
  get_vlst_cpu(bra_ptr, vlst.get(), sorb, bra_len);

  auto x = get_nsingles_doubles(no, nv, true);
  int nsingles = std::get<0>(x);
  int ndoubles = std::get<1>(x);
  int ncomb = 1 + nsingles + ndoubles;
  static std::mt19937 rng(seed);
  static std::uniform_int_distribution<int> u0(1, ncomb);
  int r = u0(rng);

  if (r == 1) {
    ;
  } else if (r > 1 && r <= nsingles + 1) {
    while (true) {
    ...
    while (true) {
      ...
      int idk = xs / no;
      if (idi == (no - 1) ||
          idk == (nv - 1)) { /* i at end of olst, s.t. i < j */
        continue;
      }
      int i = olst[idi];
      int k = vlst[idk];
      ...
      std::uniform_int_distribution<int> ud(
          0, rno * rnv - 1); /*Do not use static */
      int xd = ud(rng);
      int j = olst[xd % rno + idi + 1];
      int l = vlst[xd / rno + idk + 1];
      /***
      Doubles: (ij->kl) s.t. i < j and k < l
      1. i/k:a, l/k:b ab->ab
      2. i/k:b, l/k:b ba->ba
      3. i/k:a, l/k:b ab->ba
      4. i/k:b, l/k:a ba->ab
      ...
      ***/
      bool flag_one =
          (((i & 1) == (k & 1)) && ((j & 1) == (l & 1))); /*3, 4, 5, 6*/
      bool flag_two =
          (((i & 1) == (l & 1)) && ((j & 1) == (k & 1))); /*1, 2, 5, 6*/
      if (flag_one || flag_two) {
        BIT_FLIP(bra_ptr[i / 64], i % 64);
        BIT_FLIP(bra_ptr[j / 64], j % 64);
