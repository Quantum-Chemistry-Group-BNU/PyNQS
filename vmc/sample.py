...
class MCMCSampler():
    ...
    def read_electron_info(self, ele_info: ElectronInfo):
        print(
            f"Read electronic structure information From {ele_info.__name__}")
    ...
    def run(self, initial_state: Tensor, n_sweep: int = None) -> Tuple[Tensor, Tensor, Tensor, float, dict]:
        ...
        if self.debug_exact:
            ...
            sample_prob = torch.ones(
                dim, dtype=torch.float64, device=self.device)/dim
            return self.full_space.detach(), sample_prob, eloc, e_total, stats_dict

        t0 = time.time_ns()
        self.prepare_sample(initial_state, n_sweep)

        if self.method_sample == "MCMC":
            sample_unique, sample_counts, sample_prob, psi_unique = self.MCMC()
        else:
            sample_unique, sample_counts, sample_prob, psi_unique = self.auto_regressive()
        ...
        if self.verbose:
            print(
                f"sampling: {len(sample_counts)}/{sample_counts.sum().item()}")
        ...
        print(
            f'Completed Monte Carlo Sampling {delta/1.0E09:.3f} s, acceptance ratio = {self.n_accept/self.n_sample:.3f}')
    ...
    def MCMC(self) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
        ...
        if True:
            ...
            with torch.no_grad():
                self.n_accept = MCMC_sample(model_file, self.current_state, self.state_sample,
                                            ...
                                            self.nob, self.seed, self.n_sweep, self.therm_step)
            os.remove(model_file)
            # print(f"CPP model time: {(time.time_ns() - t0)/1.E06:.3f} ms")
        else:
            with torch.no_grad():
                psi_current = self.nqs(uint8_to_bit(
            ...
            for i in range(self.n_sweep):
                ...
                if i >= self.therm_step:
                    self.state_sample[i -
                                      self.therm_step] = self.current_state.clone()
        ...
        return sample_unique, sample_counts, sample_prob, psi_unique
    ...
    def calculate_energy(self, sample: Tensor,
                         state_prob: Tensor = None,
                         state_counts: Tensor = None):
        nbatch = get_nbatch(self.sorb, len(sample),
                            self.n_SinglesDoubles, self.max_memory, self.alpha)
