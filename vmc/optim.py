...
class VMCOptimizer():
    ...
    def __init__(self, nqs: nn.Module,
        ...
        self.sampler = MCMCSampler(
            self.model, electron_info, dtype=self.dtype, **self.sampler_param)
        self.n_sample = 0
    ...
    def run(self):
        for p in range(self.max_iter):
            ...
            if self.HF_init is None or p < self.HF_init:
                initial_state = self.onstate[random.randrange(
                    self.dim)].clone().detach()
            else:
            ...
            # lp.print_stats()
            # exit()
            state, state_prob, eloc, e_total, stats = self.sampler.run(
                initial_state)
    ...
    def pre_train(self, prefix: str = None):
        t = CITrain(self.model, self.opt, self.pre_CI,
                    self.pre_train_info, self.sorb, self.dtype, self.lr_scheduler)
        print(t)
    ...
    def plot_figure(self, e_ref: float = None, prefix: str = "VMC"):
        ...
        if e_ref is not None:
            ...
            print(
                f"reference energy: {e_ref:.9f}, error: {abs((e[-1]-e_ref)/e_ref) * 100:.6f} %")
