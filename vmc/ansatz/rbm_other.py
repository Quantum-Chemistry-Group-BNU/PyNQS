...
class ARRBM(nn.Module):
    ...
    def __init__(
        ...
        self.hidden_bias = nn.Parameter(
            (
                torch.rand(size=(self.num_hidden,), dtype=self.param_dtype, device=self.device)
                * self.iscale
                - 0.5
            )
            * 0.002
        ...
        self.weight = nn.Parameter(
            (
                torch.rand(
                    size=(self.num_hidden, self.weight_dim),
                    dtype=self.param_dtype,
                    device=self.device,
                )
                * self.iscale
        ...
        self.phase_hidden_size = [self.num_hidden * 2, self.num_hidden * 2]
    ...
    def extra_repr(self) -> str:
        s = "AR-RBM -- activate function: " + str(self.activation) + "\n"
        s += "The spin is used: " + str(self.spin) + "\n"
        s += (
            "sym is used: "
            + str(self.use_correct_size)
            + " , and the correct number:"
            + str(self.correct_size)
            + "\n"
        )
        s += "alpha factor: " + str(self.alpha) + "\n"
        s += "The number of vis_number: " + str(self.num_visible) + "\n"
        s += "Normlization: " + str(self.normal) + "\n"
        s += "Share paras: " + str(self.use_share_para)
        return s
        ...
        if state:
            if self.use_correct_size:
                if i == 0:
                    ...
                    self.vis_sym = vis_i.to(self.device)
                else:
                ...
                # sym_index = sym_index.to(self.device)
                ...
                condwf = F.normalize(condwf, dim=1, eps=1e-14)
            ind = torch.tensor([1, 2], dtype=torch.float64, device=self.device)  # (1,2)
            index = (vis_cur * ind).sum(dim=1).long()
            # index = torch.einsum("ni,i->n",vis_cur,ind).to(torch.int64) # (N, 2) * (1, 2)  -> (N, 1)
            index = torch.nn.functional.one_hot(index, num_classes=4).to(
                torch.float64
            )  # (N, 1) -> (N, 4)
    ...
    def forward(self, input: Tensor):
        ...
        if self.compute_phase:
            phase_input = (
                vis_input.masked_fill(vis_input == 0, -1).double().squeeze(1)
            )  # (nbatch, 2)
    ...
    def joint_next_sample(self, tensor: Tensor) -> Tensor:  # 用以查看产生下一个样本的可能态
        ...
        for i in range(batch_add):
            x[i * nbatch : (i + 1) * nbatch, (0 - orbital_add) :] = maybe[i].repeat(
                nbatch, 1
            )  # (nbatch, 2) -> # (batch_add * nbatch, 2)
    ...
    def ar_sampling(self, n_sample: int) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        for i in range(vis_dim):
            ...
            amp = torch.mul(wf_value.unsqueeze(1).repeat(1, self.batch_add), y0).T.flatten()[
                idx_count
            ]
