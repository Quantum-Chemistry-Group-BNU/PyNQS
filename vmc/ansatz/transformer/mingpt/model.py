...
3) https://github.com/karpathy/minGPT/tree/master
"""
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, kv_cache=None, kv_idxs=None):
        # any batch size infer of kv cache, just pass the kv_idxs
        B, T, C = x.size()  # batch size, sequence length, embedding dimensionality (n_embd)

        # calculate query, key, values for all heads in batch and move head forward to be the batch dim
        q, k, v = self.c_attn(x).split(self.n_embd, dim=2)
        # print(f"q {q.shape} k: {k.shape} v: {v.shape} B: {B} T: {T} C: {C}")
        if kv_cache is not None:
            if kv_idxs is not None:
                k_past = torch.index_select(kv_cache[0], 0, kv_idxs)
                v_past = torch.index_select(kv_cache[1], 0, kv_idxs)
            else:
        ...
        # print(f"kv_cache is None: {kv_cache is None} kv_idxs: {kv_idxs is None} k: {k}")
...
class Block(nn.Module):
    ...
    def forward(self, x, kv_cache=None, kv_idxs=None):
        # print(f"block x 1 {x.shape}")
        # x, kv_cache = x + self.attn(self.ln_1(x), kv_cache)
...
class GPT(nn.Module):
    ...
    def forward(self, idx, kv_caches=None, kv_idxs=None, targets=None):
        ...
        # print(f"{x.shape} x: {x[:,-1:,:]}")
        logits = self.lm_head(x)
        # print(f"logits: {logits.shape} {logits[:,-1:,:]}")
    ...
    def forward_org(self, idx, targets=None):
        idx = idx.type(torch.long)
        device = idx.device
        b, t = idx.size()
        assert (
...
def get_decoder_amp(
    ...
    Args:
        ...
        device: 'cpu' or 'cuda'
    """
    # P: padding, S: start, E: end
    # target word list
    tgt_vocab = {"0": 0, "1": 1, "2": 2, "3": 3, "S": 4}
    tgt_vocab_size = len(tgt_vocab) - 1  # only consider 0/1

    # length of target, e.g. ['S', x_1, x_2, ..., x_n]
    # tgt_len = n_qubits+1
    ...
    # n_layers = 6                    # number of EncoderLayer/DecoderLayer
    # n_heads  = 8                    # number of multi-heads
    # d_k      = d_model // n_heads   # dimension of Query and Key vector
    # d_v      = d_model // n_heads   # dimension of Value vector
    # d_ff     = d_model*2            # the number of FFN
    # p_dropout = 0.1                 # dropout rate

    model_config = GPT.get_default_config()
    model_config.model_type = None
    # model_config.model_type = 'gpt-nano'
    model_config.vocab_size = tgt_vocab_size
    model_config.block_size = tgt_len
...
if __name__ == "__main__":
    model = get_decoder_amp(n_qubits=8)[0]
    # x = torch.Tensor([[4,0,1,2,3]]).type(torch.long)
    x = torch.Tensor([[0, 1, 2, 3, 4], [0, 1, 2, 3, 3]]).type(torch.long)
    print(f"in: {x}")
    ...
    # kv_caches = [(torch.Tensor(), torch.Tensor()) for i in range(3)]
    # kv_caches = [(torch.Tensor(), torch.Tensor())] * 3
    for i in range(1, x.shape[-1] + 1):
        out = model(x[:, :i])
        print(f"{i}-th: x {x[:, :i]}")
        print(f"{i}-th: out {out}")
        # print(f"kv_caches: {kv_caches[i][0].shape} {kv_caches[i][1].shape}")
