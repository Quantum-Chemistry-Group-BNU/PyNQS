...
class RNNWavefunction(nn.Module):
    def __init__(self, sorb: int, num_hiddens: int, num_layers: int, num_labels: int, device: str = None):
        ...
        # input_size: spin 1/2
        self.GRU = nn.GRU(input_size=2, hidden_size=num_hiddens, num_layers=num_layers, batch_first=True, **self.factory_kwargs)
    ...
    def forward(self, x: Tensor, compute_phase: bool = True) ->Tensor:
        ...
        assert (x.dim() in (1, 2)), f"GRU: Expected input to be 1-D or 2-D but received {input.dim()}-D tensor"
        if x.dim() == 1:
            x = x.reshape(1, 1, -1)
        else:
            x = x.unsqueeze(1) 
        ...
        for i in range(dim):
            ...
            prob = (y0_amp * x0.squeeze(1)).sum(dim=1)
            if compute_phase:
                phase = (y0_phase * x0.squeeze(1)).sum(dim=1)
