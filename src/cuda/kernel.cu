...
__global__ void packbits_kernel(const uint8_t *bra, uint8_t *states,
                                     const int sorb, const int packed_bytes,
                                     const size_t n) {
  constexpr int out_bytes = _len * 8;
  size_t idn = blockIdx.x * blockDim.x + threadIdx.x;
  if (idn >= n)
    return;
  int64_t idx = idn % packed_bytes, idy = idn / packed_bytes;
  uint8_t packed = 0;
  if (idx < packed_bytes) {
    const int base = idx * 8;
#pragma unroll
    for (int i = 0; i < 8; i++) {
      const int bit = base + i;
      if (bit < sorb) {
        packed |= static_cast<uint8_t>((bra[sorb * idy + bit] & 1u) << i);
      }
    }
  }
  states[idy * out_bytes + idx] = packed;

  if (idx == packed_bytes - 1) {
    for (int i = packed_bytes; i < out_bytes; i++) {
      states[idy * out_bytes + i] = 0;
    }
  }
}

// tensor(uint8): [1, 1, 0, 0] -> onv(uint8): 0b0011, 1 occupied, 0 unoccupied
__host__ void squant::packbits_cuda(uint8_t *states, const uint8_t *bra,
                                         const int sorb, const int nbatch,
                                         const int bra_len) {
  (void)bra_len;
  dim3 blockDim(256);
  const int packed_bytes = (sorb + 7) / 8;
  const size_t n = static_cast<size_t>(nbatch) * packed_bytes;
  dim3 gridDim((n + blockDim.x - 1) / blockDim.x);
  packbits_kernel<MAX_SORB_LEN>
      <<<gridDim, blockDim>>>(bra, states, sorb, packed_bytes, n);
  cudaError_t cudaStatus = cudaGetLastError();
  HANDLE_ERROR(cudaStatus);
}

template <typename T, const int _len>
__global__ void unpackbits_kernel(T *comb, const unsigned long *bra,
  ...
  size_t idm = idn / sorb;
  int bit = idn % sorb;
  unsigned long value = bra[idm * _len + (bit >> 6)];
  comb[idn] = static_cast<T>((value >> (bit & 63)) & 1ULL);
}

// onv bytes: 0b0011 -> tensor(float/double): [1.0, 1.0, 0.0, 0.0],
// matching numpy.unpackbits(..., bitorder="little")[:, :sorb].
template <typename T>
__host__ void squant::unpackbits_cuda(T *comb, const unsigned long *bra,
  ...
  unpackbits_kernel<T, MAX_SORB_LEN>
...
__global__ void get_Hij_kernel_3D(T *Hmat, const unsigned long *bra,
  ...
  //   return;
  // Hmat[idn * m + idm] =
  //     squant::get_Hij_cuda(&bra[idn * _len], &ket[idn * m * _len + idm *
  //     _len],
  //                          h1e, h2e, sorb, nele, _len);
...
template <const int _len>
__global__ void get_merged_ovlst_kernel(const unsigned long *bra,
                                        int16_t *merged, int sorb, int nele,
                                        int n) {
  size_t idm = blockIdx.x * blockDim.x + threadIdx.x;
  if (idm >= n)
    return;
  // squant::get_ovlst_lut_cuda(&bra[idm * _len], &merged[idm * sorb], sorb,
  // nele,
  //                        _len);
...
__host__ void squant::get_merged_cuda(const unsigned long *bra, int16_t *merged,
...
template <const int _len, typename T>
__global__ void get_comb_SD_kernel(unsigned long *comb, T *comb_bit,
                                   const int16_t *merged, int sorb, int noA,
  ...
  squant::get_comb_SD_cuda<T>(&comb[idx * ncomb * _len + idy * _len],
                              &comb_bit[idx * ncomb * sorb + idy * sorb],
                              _merged_sh, idy - 1, sorb, noA, noB);
}

template <const int _len>
__global__ void get_comb_SD_kernel(unsigned long *comb, const int16_t *merged,
...
get_comb_SD_fused_kernel(unsigned long *bra, unsigned long *comb,
                         const int16_t *merged, const T *h1e, const T *h2e,
                         T *Hmat, const int sorb, const int noA, const int noB,
                         const int nbatch, const int ncomb) {
  __shared__ int _merged_sh[MAX_SORB_LEN * 64];
...
__host__ void squant::get_comb_fused_cuda(
    unsigned long *bra, unsigned long *comb, const int16_t *merged,
    const T *h1e, const T *h2e, T *Hmat, const int sorb, const int len,
    const int noA, const int noB, const int nbatch, const int ncomb) {
  dim3 blockDim(256);
...
__host__ void squant::get_comb_cuda(unsigned long *comb,
                                    const int16_t *merged_ovlst, const int sorb,
...
template <typename T>
__host__ void squant::get_comb_cuda(T *comb_bit, unsigned long *comb,
                                    const int16_t *merged_ovlst, const int sorb,
                                    const int bra_len, const int noA,
                                    const int noB, const int nbatch,
                                    const int ncomb) {
  ...
  dim3 blockDim(256);
  dim3 gridDim(nbatch * ((ncomb - 1) / blockDim.x + 1));
  get_comb_SD_kernel<MAX_SORB_LEN, T><<<gridDim, blockDim>>>(
...
squant::get_Hij_2D_cuda<double>(double *, const unsigned long *,
                                const unsigned long *, const double *,
                                const double *, int, int, int, int, int);
template __host__ void squant::get_comb_fused_cuda<double>(
    unsigned long *, unsigned long *, const int16_t *, const double *,
    const double *, double *, int, int, int, int, int, int);

template __host__ void
squant::unpackbits_cuda<double>(double *comb, const unsigned long *bra,
                                const int sorb, const int bra_len,
                                const int nbatch, const size_t numel);

template __host__ void squant::get_comb_cuda<double>(
    double *comb_bit, unsigned long *comb, const int16_t *merged_ovlst,
    const int sorb, const int bra_len, const int noA, const int noB,
    const int nbatch, const int ncomb);
...
squant::get_Hij_2D_cuda<float>(float *, const unsigned long *,
                               const unsigned long *, const float *,
                               const float *, int, int, int, int, int);
template __host__ void squant::get_comb_fused_cuda<float>(
    unsigned long *, unsigned long *, const int16_t *, const float *,
    const float *, float *, int, int, int, int, int, int);

template __host__ void
squant::unpackbits_cuda<float>(float *comb, const unsigned long *bra,
                               const int sorb, const int bra_len,
                               const int nbatch, const size_t numel);

template __host__ void squant::get_comb_cuda<float>(
    float *comb_bit, unsigned long *comb, const int16_t *merged_ovlst,
    const int sorb, const int bra_len, const int noA, const int noB,
    const int nbatch, const int ncomb);
...
__device__ int64_t BigInteger_device(const IntType *arr, const IntType *target,
  ...
  while (left <= right) {
    ...
    int result = compare<IntType, _len>(mid_element, target, little_endian);
...
// This is pretty slower using philox method
// aten/src/ATen/native/cuda/Randperm.cuh
template <int _len>
__global__ void
spin_flip_rand_kernel(unsigned long *bra, const int32_t *merged, const int sorb,
                      const int nele, const int noA, const int noB,
                      const int64_t nbatch, const int ncomb, const int seed) {
