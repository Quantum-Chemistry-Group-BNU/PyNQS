...
#include <cstddef>
#include <cstdio>
#include "bitmask_constants_8bit.h"
namespace squant {
...
__device__ void get_ovlst_cuda(const unsigned long *bra, int16_t *merged,
  ...
  int ida = 0;
  int idb = 0;
  const int last_bits = sorb % 64 == 0 ? 64 : sorb % 64;
  const unsigned long tail_mask = get_ones_cuda(last_bits);
  const int16_t even_stride = 2;

  // occupied orbital
  for (int i = 0; i < bra_len; i++) {
    const unsigned long occ = bra[i] & ((i == bra_len - 1) ? tail_mask : ~0ULL);
    const int base = i << 6;
    tmp = occ & kEvenBitMask;
    while (tmp != 0) {
      const int j = __ffsll(static_cast<long long>(tmp)) - 1;
      merged[ida * even_stride] = static_cast<int16_t>(base + j);
      ++ida;
      tmp &= tmp - 1ULL;
    }
    tmp = occ & kOddBitMask;
    while (tmp != 0) {
      const int j = __ffsll(static_cast<long long>(tmp)) - 1;
      merged[idb * even_stride + 1] = static_cast<int16_t>(base + j);
      ++idb;
      tmp &= tmp - 1ULL;
    }
  }
  ...
  // virtual orbital
  for (int i = 0; i < bra_len; i++) {
    const unsigned long virt = (~bra[i]) & ((i == bra_len - 1) ? tail_mask : ~0ULL);
    const int base = i << 6;
    tmp = virt & kEvenBitMask;
    while (tmp != 0) {
      const int j = __ffsll(static_cast<long long>(tmp)) - 1;
      merged[ida * even_stride] = static_cast<int16_t>(base + j);
      ++ida;
...
__device__ void get_ovlst_lut_cuda(const unsigned long *bra,
                               int *merged,
                               const int sorb,
                               const int nele,
                               const int bra_len) {
    int ida = 0, idb = 0;
    // TODO: how to implement? this version is error
    for (int i = 0; i < bra_len; i++) {
        unsigned long occ = bra[i];
        unsigned long virt = (i != bra_len - 1)
            ? ~bra[i]
            : (~bra[i] & get_ones_cuda(sorb % 64 == 0 ? 64 : sorb % 64));

        int bytes_in_block = (i != bra_len - 1) ? 8 : ((sorb % 64 == 0) ? 8 : (sorb % 64 + 7)/8);
        // 64-bit 8-byte
        for (int byte = 0; byte < bytes_in_block; byte++) {
            int shift = byte * 8;
            unsigned char occ8 = (occ >> shift) & 0xFF;
            unsigned char virt8 = (virt >> shift) & 0xFF;
            // occupied 
            int cnt = COUNT_8BIT[occ8];
            for (int k = 0; k < cnt; k++) {
                int j = INDEX_8BIT[occ8][k] + shift;
                int s = i * 64 + j;
                bool is_beta = s & 1;
                int pos = is_beta ? idb++ : ida++;
                int idx = (pos << 1) | is_beta;
                merged[idx] = s;
            }
            // virtual
            cnt = COUNT_8BIT[virt8];
            for (int k = 0; k < cnt; k++) {
                int j = INDEX_8BIT[virt8][k] + shift;
                int s = i * 64 + j;
                bool is_beta = s & 1;
                int pos = is_beta ? idb++ : ida++;
                int idx = (pos << 1) | is_beta;
