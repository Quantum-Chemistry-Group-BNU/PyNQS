...
struct KeyT {
  ...
  __device__ __host__ explicit KeyT(const unsigned long *values) {
    uint64_t *ptr = reinterpret_cast<uint64_t *>((void *)data);
#pragma unroll
    for (int i = 0; i < MAX_SORB_LEN; ++i) {
      ptr[i] = static_cast<uint64_t>(values[i]);
    }
  }

  template <typename... Args,
            typename = std::enable_if_t<
                (sizeof...(Args) == MAX_SORB_LEN) &&
                std::conjunction_v<std::is_same<std::decay_t<Args>, uint64_t>...>>>
  __device__ __host__ explicit KeyT(Args... values) {
    uint64_t *ptr = reinterpret_cast<uint64_t *>((void *)data);
    size_t index = 0;
    ((ptr[index++] = values), ...);
  }
