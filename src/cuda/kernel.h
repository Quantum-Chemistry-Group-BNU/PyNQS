...
__host__ void packbits_cuda(uint8_t *states, const uint8_t *bra,
                                 const int sorb, const int nbatch,
                                 const int bra_len);

// bra: (nbatch, onv bytes), comb: (nbatch, sorb) float/double 0/1
template <typename T>
__host__ void unpackbits_cuda(T *comb, const unsigned long *bra,
                              const int sorb, const int bra_len,
                              const int nbatch, const size_t numel);

// merge olst and vlst, bra: (nbatch, onv)
__host__ void get_merged_cuda(const unsigned long *bra, int16_t *merged,
...
// comb_bit: (nbatch, ncomb, sorb) float/double 0/1
// comb: (nbatch, ncomb, onv), merged_ovlst: (nbatch, sorb)
template <typename T>
__host__ void get_comb_cuda(T *comb_bit, unsigned long *comb,
                            const int16_t *merged_ovlst, const int sorb,
                            const int bra_len, const int noA, const int noB,
                            const int nbatch, const int ncomb);

// comb: (nbatch, ncomb, onv), merged_ovlst: (nbatch, sorb)
__host__ void get_comb_cuda(unsigned long *comb, const int16_t *merged_ovlst,
...
__host__ void get_comb_fused_cuda(unsigned long *bra, unsigned long *comb,
                                  const int16_t *merged, const T *h1e,
                                  const T *h2e, T *Hmat,
