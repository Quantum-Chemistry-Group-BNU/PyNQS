...
Tensor packbits_tensor_cpu(const Tensor &bra_tensor, const int sorb) {
  // numpy.packbits(..., bitorder="little") padded to 64-bit words.
  // bra_tensor(nbatch, sorb) uint8: [1, 1, 0, 0] -> 0b0011 uint8
  // return states: (nbatch, bra_len * 8)
  const int bra_len = (sorb - 1) / 64 + 1;
  const int packed_bytes = (sorb + 7) / 8;
  const int out_bytes = bra_len * 8;
  TORCH_CHECK(bra_tensor.dtype() == torch::kUInt8,
              "packbits_tensor_cpu expects a torch.uint8 tensor");
  auto dim = bra_tensor.dim();
  TORCH_CHECK(dim == 2 || dim == 1,
              "packbits_tensor_cpu expects a 1D or 2D tensor");
  TORCH_CHECK(bra_tensor.size(-1) == sorb,
              "packbits_tensor_cpu expects the last dimension to equal sorb");
  int64_t nbatch = 1;
  if (dim == 2) {
  ...
  if (bra_tensor.numel() == 0) {
    return torch::empty({0, out_bytes}, options);
  }
  Tensor states = torch::zeros({nbatch, out_bytes}, options);
    ...
    for (const auto i : at::irange(begin, end)) {
      for (int64_t j = 0; j < packed_bytes; j++) {
        ...
        for (int64_t k = 0; k < 8; k++) {
          ...
          if (bit < sorb) {
            packed |= static_cast<uint8_t>((bra_ptr[i * sorb + bit] & 1u) << k);
          }
        }
        states_ptr[i * out_bytes + j] = packed;
      }
    }
  });
  return states;
...
torch::Tensor unpackbits_tensor_cpu(const torch::Tensor &bra_tensor,
                                       const int sorb) {
  // numpy.unpackbits(..., bitorder="little")[:, :sorb].
  // bra_tensor(nbatch, bra_len) uint8: 0b0011 -> [1.0, 1.0, 0.0, 0.0]
  // return comb_bit: (nbatch, sorb), dtype=torch.get_default_dtype().
  ...
  TORCH_CHECK(input_bytes % 8 == 0,
              "unpackbits_tensor_cpu expects ONV bytes padded to 8-byte blocks");
  TORCH_CHECK(input_bytes >= needed_bytes,
              "bra_tensor has fewer bytes than required by sorb");
  const uint8_t *bra_ptr = bra_tensor.data_ptr<uint8_t>();

  AT_DISPATCH_FLOATING_TYPES(
      comb_bit.scalar_type(), "unpackbits_tensor_cpu", ([&] {
        auto *comb_ptr = comb_bit.data_ptr<scalar_t>();
        at::parallel_for(0, nbatch, 0, [&](int64_t begin, int64_t end) {
          for (const auto i : c10::irange(begin, end)) {
            for (int64_t j = 0; j < sorb; j++) {
              const uint8_t packed = bra_ptr[i * input_bytes + (j >> 3)];
              comb_ptr[i * sorb + j] =
                  static_cast<scalar_t>((packed >> (j & 7)) & 1u);
            }
          }
        });
      }));
...
tuple_tensor_2d spin_flip_rand_cpu(const Tensor &bra_tensor, const int sorb,
  ...
  int64_t ncomb = 0;
  if (include_D) {
    ncomb = squant::get_Num_SinglesDoubles(sorb, noA, noB);
  }
  else {
    int k = sorb / 2;
  ...
  auto rand_idx = torch::randint(0, ncomb + 1, {nbatch}, gen_);  // [0, ncomb]
  auto rand_ptr = rand_idx.data_ptr<int64_t>();
  at::parallel_for(0, nbatch, 0, [&](int64_t begin, int64_t end) {
    for (const auto i : c10::irange(begin, end)) {
  ...
  return std::make_tuple(
      unpackbits_tensor_cpu(bra.reshape({nbatch, -1}), sorb), bra);
}
...
tuple_tensor_2d get_comb_tensor_cpu(const Tensor &bra_tensor, const int sorb,
  ...
  if (flag_bit) {
    ...
    comb_bit = unpackbits_tensor_cpu(bra_tensor, sorb)
                   .unsqueeze(1)
                   .repeat({1, ncomb, 1});
  } else {
  ...
  if (flag_bit) {
    AT_DISPATCH_FLOATING_TYPES(
        comb_bit.scalar_type(), "get_comb_tensor_cpu", ([&] {
          auto *comb_bit_ptr = comb_bit.data_ptr<scalar_t>();
          at::parallel_for(0, nbatch, 0, [&](int64_t begin, int64_t end) {
            for (const auto i : c10::irange(begin, end)) {
              for (int64_t j = 1; j < ncomb; j++) {
                // comb_bit stores unpackbits-style 0/1 floats; flipping a
                // determinant bit maps v -> 1 - v.
                squant::get_comb_SD<scalar_t>(
                    &comb_ptr[i * ncomb * bra_len + j * bra_len],
                    &comb_bit_ptr[i * ncomb * sorb + j * sorb],
                    &merged_ptr[i * sorb], j - 1, sorb, bra_len, noA, noB);
              }
            }
          });
        }));
