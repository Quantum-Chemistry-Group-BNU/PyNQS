...
#include "tensor/cpu_tensor.h"
#include "torch/types.h"
#include "utils_tensor.h"
// #include "tensor/cpu_tensor.h"

#include "cuda/spin_flip.h"
#include <mutex>

Tensor packbits_tensor_cuda(const Tensor &bra_tensor, const int sorb) {
  const int bra_len = (sorb - 1) / 64 + 1;
  auto nbatch = bra_tensor.size(0);
  TORCH_CHECK(bra_tensor.dtype() == torch::kUInt8,
              "packbits_tensor_cuda expects a torch.uint8 tensor");
  TORCH_CHECK(bra_tensor.dim() == 2,
              "packbits_tensor_cuda expects a 2D tensor");
  TORCH_CHECK(bra_tensor.size(1) == sorb,
              "packbits_tensor_cuda expects the last dimension to equal sorb");
  ...
  Tensor states = torch::empty({nbatch, bra_len * 8}, options = options);
  const uint8_t *bra_ptr = bra_tensor.data_ptr<uint8_t>();
  uint8_t *states_ptr = states.data_ptr<uint8_t>();
  squant::packbits_cuda(states_ptr, bra_ptr, sorb, nbatch, bra_len);
  // C10_CUDA_KERNEL_LAUNCH_CHECK();
  return states;
}

Tensor unpackbits_tensor_cuda(const Tensor &bra_tensor, const int sorb) {
  const auto nbatch = bra_tensor.size(0);
  TORCH_CHECK(bra_tensor.dim() == 2, "bra_tensor must be 2D");
  TORCH_CHECK(bra_tensor.dtype() == torch::kUInt8,
              "unpackbits_tensor_cuda expects a torch.uint8 tensor");
  const auto input_bytes = bra_tensor.size(1);
  const auto needed_bytes = (sorb + 7) / 8;
  TORCH_CHECK(input_bytes % 8 == 0,
              "unpackbits_tensor_cuda expects ONV bytes padded to 8-byte blocks");
  TORCH_CHECK(input_bytes >= needed_bytes,
              "bra_tensor has fewer bytes than required by sorb");
  const int bra_len = input_bytes / 8;
  TORCH_CHECK(bra_len == MAX_SORB_LEN,
              "unpackbits_tensor_cuda expects ONV length to match MAX_SORB_LEN");
  ...
  const unsigned long *bra_ptr =
      reinterpret_cast<const unsigned long *>(bra_tensor.data_ptr<uint8_t>());
  AT_DISPATCH_FLOATING_TYPES(
      comb_bit.scalar_type(), "unpackbits_tensor_cuda", ([&] {
        auto *comb_ptr = comb_bit.data_ptr<scalar_t>();
        squant::unpackbits_cuda<scalar_t>(
...
namespace {
inline int64_t get_spin_flip_ncomb_cuda(const int sorb, const int noA,
                                        const int noB,
                                        const bool include_D) {
  if (include_D) {
    return squant::get_Num_SinglesDoubles_cuda(sorb, noA, noB);
  }
  const int k = sorb / 2;
  const int nvA = k - noA;
  const int nvB = k - noB;
  const int nSa = noA * nvA;
  const int nSb = noB * nvB;
  return static_cast<int64_t>(nSa + nSb);
}

tuple_tensor_2d spin_flip_rand_cuda_impl(const Tensor &bra_tensor, const int sorb,
                                         const int nele, const int noA,
                                         const int noB, const bool in_place,
                                         const bool include_D,
                                         std::optional<at::Generator> &gen_,
                                         const bool use_direct) {
  auto bra = in_place ? bra_tensor : bra_tensor.clone();
  if (bra.dim() == 1) {
    bra = bra.reshape({1, -1});
  }
  ...
  const int64_t ncomb = get_spin_flip_ncomb_cuda(sorb, noA, noB, include_D);
  const int64_t nbatch = bra.size(0);
  auto gen = at::get_generator_or_default<at::CUDAGeneratorImpl>(
      gen_, at::cuda::detail::getDefaultCUDAGenerator());

  const int64_t counter_offset = nbatch;
  ...
  if (use_direct) {
    spin_flip_rand_philox_direct_impl(bra_ptr, sorb, noA, noB, nbatch, ncomb,
                                      rng_engine_inputs);
  } else {
    Tensor merged = get_merged_tensor_cuda(bra, nele, sorb, noA, noB);
    auto *merged_ptr = merged.data_ptr<int16_t>();
    spin_flip_rand_philox_impl(bra_ptr, merged_ptr, sorb, nele, noA, noB,
                               nbatch, ncomb, rng_engine_inputs);
  }

  return std::make_tuple(
      unpackbits_tensor_cuda(bra.reshape({nbatch, -1}), sorb), bra);
}
}  // namespace

tuple_tensor_2d spin_flip_rand_cuda_direct(const Tensor &bra_tensor,
                                           const int sorb, const int nele,
                                           const int noA, const int noB,
                                           const bool in_place,
                                           const bool include_D,
                                           std::optional<at::Generator> &gen_) {
  return spin_flip_rand_cuda_impl(bra_tensor, sorb, nele, noA, noB, in_place,
                                  include_D, gen_, true);
}

tuple_tensor_2d spin_flip_rand_cuda_merged(const Tensor &bra_tensor,
                                           const int sorb, const int nele,
                                           const int noA, const int noB,
                                           const bool in_place,
                                           const bool include_D,
...
tuple_tensor_2d spin_flip_rand_cuda(const Tensor &bra_tensor, const int sorb,
                                    ...
                                    const bool include_D,
                                    std::optional<at::Generator> &gen_) {
  return spin_flip_rand_cuda_direct(bra_tensor, sorb, nele, noA, noB, in_place,
                                    include_D, gen_);
}
...
Tensor get_merged_tensor_cuda(const Tensor &bra, const int nele, const int sorb,
  ...
  Tensor bra_2d = bra;
  if (bra_2d.dim() == 1) {
    bra_2d = bra_2d.reshape({1, -1});
  }
  TORCH_CHECK(bra_2d.dim() == 2, "bra tensor must be 1D or 2D");

  const int bra_len = (sorb - 1) / 64 + 1;
  const int64_t nbatch = bra_2d.size(0);
  auto options = torch::TensorOptions()
                     .dtype(torch::kInt16)
                     .layout(bra_2d.layout())
                     .device(bra_2d.device())
                     .requires_grad(false);
  if (bra_2d.numel() == 0) {
    return torch::empty({0, sorb}, options);
  }
  torch::Tensor merged = torch::empty({nbatch, sorb}, options);
  int16_t *merged_ptr = merged.data_ptr<int16_t>();
  unsigned long *bra_ptr =
      reinterpret_cast<unsigned long *>(bra_2d.data_ptr<uint8_t>());
...
tuple_tensor_2d get_comb_tensor_cuda(const Tensor &bra_tensor, const int sorb,
  ...
  if (bra_tensor.numel() == 0) {
    ...
    if (flag_bit) {
      comb_bit = torch::empty(
          {0, ncomb, sorb},
          torch::TensorOptions().dtype(torch::get_default_dtype()).device(device));
    } else {
      comb_bit = torch::empty(
          {0, ncomb, sorb},
  ...
  if (flag_bit) {
    ...
    comb_bit = unpackbits_tensor_cuda(bra_tensor, sorb)
                   .unsqueeze(1)
                   .repeat({1, ncomb, 1});
    // torch::cuda::synchronize();
  } else {
  ...
  if (flag_bit) {
    AT_DISPATCH_FLOATING_TYPES(
        comb_bit.scalar_type(), "get_comb_tensor_cuda", ([&] {
          squant::get_comb_cuda<scalar_t>(comb_bit.data_ptr<scalar_t>(),
                                          comb_ptr, merged_ptr, sorb, bra_len,
