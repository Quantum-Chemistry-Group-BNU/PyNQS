...
Tensor packbits(const Tensor &bra_tensor, const int sorb) {
  const auto dim = bra_tensor.dim();
  CHECK_CONTIGUOUS(bra_tensor);
  TORCH_CHECK(dim == 1 || dim == 2, "packbits expects a 1D or 2D tensor");
  TORCH_CHECK(bra_tensor.dtype() == torch::kUInt8,
              "packbits expects a torch.uint8 tensor");
  TORCH_CHECK(bra_tensor.size(-1) == sorb,
              "packbits expects the last dimension to equal sorb");

  if (bra_tensor.is_cpu()) {
    return packbits_tensor_cpu(bra_tensor.view({-1, sorb}), sorb);
#ifdef GPU
  } else {
    return packbits_tensor_cuda(bra_tensor.view({-1, sorb}), sorb);
#endif
  }
}

Tensor unpackbits(const Tensor &bra_tensor, const int sorb) {
  const auto dim = bra_tensor.dim();
  const auto bra_len = bra_tensor.size(-1);
  CHECK_CONTIGUOUS(bra_tensor);
  TORCH_CHECK(dim == 1 || dim == 2, "unpackbits expects a 1D or 2D tensor");
  TORCH_CHECK(bra_tensor.dtype() == torch::kUInt8,
              "unpackbits expects a torch.uint8 tensor");
  TORCH_CHECK(bra_len % 8 == 0,
              "unpackbits expects the ONV byte dimension to be a multiple of 8");
  TORCH_CHECK(bra_len >= (sorb + 7) / 8,
              "unpackbits input has fewer bytes than required by sorb");
  if (bra_tensor.is_cpu()) {
    return unpackbits_tensor_cpu(bra_tensor.view({-1, bra_len}), sorb);
#ifdef GPU
  } else {
    return unpackbits_tensor_cuda(bra_tensor.view({-1, bra_len}), sorb);
#endif
  }
}
...
auto MCMC_sample(const std::string model_file, Tensor &initial_state,
  ...
  std::vector<torch::jit::IValue> inputs = {
      unpackbits(current_state, sorb).view({-1})};
...
tuple_tensor_2d spin_flip_rand(const Tensor &bra_tensor, const int sorb,
  ...
  CHECK_CONTIGUOUS(bra_tensor);
  if (bra_tensor.is_cpu()) {
    return spin_flip_rand_cpu(bra_tensor, sorb, nele, noA, noB,  in_place, include_D, 
                              gen_);
#ifdef GPU
  } else {
    return spin_flip_rand_cuda(bra_tensor, sorb, nele, noA, noB, in_place, include_D, 
                               gen_);
#endif
  }
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("get_hij_torch", &get_Hij, py::arg("bra"), py::arg("ket"),
        ...
        "Calculate the matrix <x|H|x'> using CPU or GPU , support Float32 and "
        "Double");
  ...
  m.def("unpackbits", &unpackbits, py::arg("bra"), py::arg("sorb"),
        "unpack onv bytes to float/double occupation bits (0: unoccupied, 1: "
        "occupied) using numpy.unpackbits(..., bitorder='little') semantics "
        "and torch.get_default_dtype()");
  m.def(
      "spin_flip_rand",
    [](const Tensor &bra_tensor, int sorb, int nele, int noA, int noB,
       bool include_D, bool in_place,
       std::optional<at::Generator> gen_opt) -> tuple_tensor_2d {
        return spin_flip_rand(bra_tensor, sorb, nele, noA, noB, include_D, in_place,
                              gen_opt);
    },
      "Flip the spin randomly in MCMC using CPU or CUDA", py::arg("bra"),
      py::arg("sorb"), py::arg("nele"), py::arg("noA"), py::arg("noB"),
      py::kw_only(), py::arg("include_D") = true, py::arg("in_place") = false,
      py::arg("generator") = std::nullopt);
#ifdef GPU
  m.def(
      "spin_flip_rand_cuda_direct",
      [](const Tensor &bra_tensor, int sorb, int nele, int noA, int noB,
         bool include_D, bool in_place,
         std::optional<at::Generator> gen_opt) -> tuple_tensor_2d {
        TORCH_CHECK(bra_tensor.is_cuda(),
                    "spin_flip_rand_cuda_direct expects a CUDA tensor");
        return spin_flip_rand_cuda_direct(bra_tensor, sorb, nele, noA, noB,
                                          in_place, include_D, gen_opt);
      },
      "CUDA only spin flip (direct rank->orbital)", py::arg("bra"),
      py::arg("sorb"), py::arg("nele"), py::arg("noA"), py::arg("noB"),
      py::kw_only(), py::arg("include_D") = true, py::arg("in_place") = false,
      py::arg("generator") = std::nullopt);

  m.def(
      "spin_flip_rand_cuda_merged",
      [](const Tensor &bra_tensor, int sorb, int nele, int noA, int noB,
         bool include_D, bool in_place,
         std::optional<at::Generator> gen_opt) -> tuple_tensor_2d {
        TORCH_CHECK(bra_tensor.is_cuda(),
                    "spin_flip_rand_cuda_merged expects a CUDA tensor");
        return spin_flip_rand_cuda_merged(bra_tensor, sorb, nele, noA, noB,
                                          in_place, include_D, gen_opt);
      },
      "CUDA only spin flip (build merged table then lookup)", py::arg("bra"),
      py::arg("sorb"), py::arg("nele"), py::arg("noA"), py::arg("noB"),
      py::kw_only(), py::arg("include_D") = true, py::arg("in_place") = false,
      py::arg("generator") = std::nullopt);
#endif
  m.def("packbits", &packbits, py::arg("bra"), py::arg("sorb"),
        "pack uint8 occupation bits (0: unoccupied, 1: occupied) to onv bytes "
        "using numpy.packbits(..., bitorder='little') semantics");
  ...
  m.def("decompress_h1e_h2e", &decompress_h1e_h2e, py::arg("h1e"),
        py::arg("h2e"), py::arg("sorb"), "Decompress h1e/h2e(1D) to 2D/4D");
}
