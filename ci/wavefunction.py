...
class CITrain:
    ...
    def __init__(self, model: nn.Module,
        ...
        self.state = pt.uint8_to_bit(self.onstate, self.sorb)
        self.pre_max_iter = pre_train_info["pre_max_iter"]
        if int(pre_train_info["interval"]) != -1:
        ...
        if not self.loss_type in self.LOSS_TYPE:
            raise ValueError(
                f"Loss function type{self.loss_type} not in {self.LOSS_TYPE}, ")

    def train(self, prefix: str = None, 
        ...
        if True:
            full_space = pt.uint8_to_bit(electron_info.ci_space, self.sorb)
        ...
        print(
            f"Pre-train finished, cost time: {(time.time_ns() - begin)/1.E09:.3f}s")
        if flag_energy:
            print(
                f"Energy ref, before/after pre_training: {eCI_ref:.8f} {eCI_0:.8f}, {eCI_1:.8f}")
