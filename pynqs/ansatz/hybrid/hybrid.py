...
from pynqs.ansatz import RBMWavefunction, DecoderWaveFunction
...
class HybridWaveFunction(nn.Module):
    ...
    def hybrid_forward(self, x: Tensor, *keys, **kwargs) -> Tensor:
        x_occ = x.to(x.dtype)
        amp = self.amp_layers(x_occ, *keys, **kwargs)
        phase = self.phase_comb(1 - x_occ)  # legacy -spin phase input, represented as 0/1
        # amp * exp(i * phase)
        wf = amp * phase
