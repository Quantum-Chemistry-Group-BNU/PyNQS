...
from pynqs.utils import get_fock_space
...
class IsingRBM(nn.Module):
    ...
    def __init__(
        ...
        self.param_dtype = dtype_config.real_dtype
    ...
    def forward(self, x: Tensor):
        x = x.to(self.param_dtype) * 2 - 1  # model input 0/1 -> spin input -1/1
...
class DBM(nn.Module):
    ...
    def __init__(
        ...
        self.param_dtype = torch.double
        if self.use_complex:
            self.param_dtype = torch.complex128
    ...
    def forward(self, x: Tensor):
        x = x.to(self.param_dtype) * 2 - 1  # model input 0/1 -> spin input -1/1
        h = unpackbits(get_fock_space(self.num_hidden[0]), self.num_hidden[0]).to(self.device)
        h = h * 2 - 1  # hidden RBM sum still uses spin states
...
class Jastrow(nn.Module):
    ...
    def __init__(self, nqubits, prod_dim=1, device="cuda", iscale: float = 0.001, use_complex=False):
        ...
        self.iscale = iscale
        self.use_complex = use_complex
        self.factory_kwargs_real = {"device": self.device, "dtype": dtype_config.real_dtype}
    ...
    def forward(self, x):
        x = x.to(dtype_config.real_dtype) * 2 - 1  # model input 0/1 -> spin input -1/1
        if self.use_complex:
...
class mlp_linear(nn.Module):
    ...
    def init_from_file(
        ...
        for key, param in x.items():
            ...
            if not key1.startswith("params_"):
                key1 = "params_" + key1
            if key1 in KEYS:
                params_dict[key1] = param.to(dtype_config.real_dtype)
    ...
    def forward(self, x: Tensor):
        ...
        wf = (
            torch.einsum("ij,ni->nj", self.M1, 2 * torch.pi * self.f0(hidden_0)) + self.b1
        )  # (nbatch, hidden1)
        if len(self.hidden_list) > 2:
