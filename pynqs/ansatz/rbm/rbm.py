...
class RBMWavefunction(nn.Module):
    ...
    def __init__(
        ...
        self.default_type = dtype_config.real_dtype
    ...
    def params_complex(self, init_weight: float, num_hidden: int, num_visible: int):
        if self.rbm_type == "cos":
            ...
            visible_bias = (
                init_weight * 100 * (torch.rand(num_visible, 2, device=self.device, dtype=self.default_type))
                - 0.5
            )

        # hidden-bias
        hidden_bias = init_weight * (
            torch.rand(num_hidden, 2, device=self.device, dtype=self.default_type) - 0.5
        )

        # hidden-bias
        weights = init_weight * (
            torch.rand(num_hidden, num_visible, 2, device=self.device, dtype=self.default_type) - 0.5
        )
    ...
    def params_real(self, init_weight: float, num_hidden: int, num_visible: int):
        if self.rbm_type == "cos":
            ...
            visible_bias = (
                init_weight
                * 100
                * (torch.rand(num_visible, device=self.device, dtype=self.default_type) - 0.5)
            )
    ...
    def psi(self, x: Tensor) -> Tensor:
        ...
        x = x.to(self.default_type) * 2 - 1  # model input 0/1 -> RBM spin input -1/1
        if self.rbm_type == "complex":
