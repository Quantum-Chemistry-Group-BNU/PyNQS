...
from pynqs.utils.public_function import multinomial_tensor, torch_lexsort
from pynqs.libs.C_extension import constrain_make_charts
...
class RBMSites(nn.Module):
    ...
    def __init__(
        ...
        if activation_type not in self.ACTIVATION_TYPE:
            raise TypeError(f"Activate type : Expected {self.ACTIVATION_TYPE} but received {activation_type}")
    ...
    def forward_two_sites(
        ...
        if use_unique:
            ...
            unique_sorb = min(int(torch.tensor(nbatch / 1024 + 1).log2().ceil() + 1), self.sorb // 2)
            unique_sorb = max(2, unique_sorb)
            # sorted x, avoid repeated sorting using 'torch.unique'
            sorted_idx = torch_lexsort(
        ...
        for k in range(0, self.sorb, 2):
            ...
            # else:
            index = F.one_hot((x[:, k : k + 2] * baselines).sum(dim=1).long(), num_classes=4).to(torch.double)
            prob_k = (y0 * index).sum(dim=1)
            # avoid In-place when auto-grad
            prob = torch.mul(prob, prob_k)
            # prob_lst.append(prob_k)
