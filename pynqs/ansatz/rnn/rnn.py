...
from pynqs.ansatz.utils import OrbitalBlock
...
class RNNWavefunction(nn.Module):
    ...
    def forward(
        ...
        if use_unique:
            ...
            sorted_idx = torch_lexsort(
                keys=list(map(torch.flatten, reversed(x.squeeze(1)[:, :unique_sorb].split(1, dim=1))))
            )
            x = x[sorted_idx]
            ...
            if use_unique:
                ...
                if i <= unique_sorb:
                    ...
                    if i == 0:
                        ...
                        inverse_i, index_i = torch_consecutive_unique_idex(x[..., :i].squeeze(1), dim=0)[1:3]
                        x0 = x0[index_i]
                    if i == 0:
    ...
    def ar_sampling(self, n_sample: int) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        for i in range(self.nqubits):
            ...
            x0 = (
                F.one_hot(sample_unique[..., i], num_classes=2).to(**self.factory_kwargs).unsqueeze(1)
            )  # (n_unique, 1, 2)

        if self.compute_phase and not self.combine_amp_phase:
            phase_input = (sample_unique * 2 - 1).double().squeeze(1)  # [0, 1] -> [-1, 1]
            phase_i = self.phase_layers[0](phase_input)
            if self.n_out_phase == 1:
                phase = phase_i.view(-1)
            else:
