...
from pynqs.ansatz.utils import joint_next_samples, OrbitalBlock
    ...
    def param_init_one_site(self):
        if self.param_dtype == torch.complex128:
            ...
            self.parm_M_v = nn.Parameter(
                torch.zeros(self.L, self.M, self.hilbert_local, self.dcut, self.dcut, device=self.device)
                * self.iscale
            )
    ...
    def param_init_two_site(self):
        if self.param_dtype == torch.complex128:
            ...
            if self.params_file is not None:
                ...
                self.parm_v = nn.Parameter(
                    torch.randn(self.L, self.M // 2, self.hilbert_local, self.dcut, **self.factory_kwargs)
                    * self.iscale
                )
                if self.use_tensor:
                    self.parm_T = nn.Parameter(
