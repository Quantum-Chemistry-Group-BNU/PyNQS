...
from pynqs.ansatz.transformer.nanogpt.model import GPT, GPTConfig, get_decoder_amp
...
from pynqs.ansatz.symmetry import symmetry_mask, orthonormal_mask
...
class DecoderWaveFunction(nn.Module):
    ...
    def __init__(
        ...
        if wf_type == "complex":
            ...
            raise TypeError(f"Transformer-Decoder-nqs types({wf_type}) must be in ('complex', 'real')")
    ...
    def _get_conditional_output(
        ...
        x_occ = x.to(x.dtype)
        amp_input = x_occ
        phase_input = (x_occ * 2 - 1).to(torch.double)
        ...
        amp_i = self.amp_layers(amp_input.long(), kv_caches=kv_caches, kv_idxs=kv_idxs)  # (nbatch, 4/2)
        ...
        if x0.size(0) < min_batch or min_batch < 0:
            ...
            if self.use_kv_cache:
                for i in range(kv_length):
                    ...
                    k_cache = torch.cat([_kv_cache[i][0] for _kv_cache in kv_caches_lst], dim=0)
                    v_cache = torch.cat([_kv_cache[i][1] for _kv_cache in kv_caches_lst], dim=0)
    ...
    def _interval_sample(
        ...
        for k in range(begin, end, interval):
            x0 = sample_unique
            amp = self.batch_get_amps(x0, k=k, min_batch=min_batch, kv_caches=kv_caches, kv_idxs=kv_idxs)
            ...
            t0 = time.time_ns()
            amp_orth_mask = self.orth_mask(states=sample_unique, k=k, num_up=num_up, num_down=num_down)
            ...
            if self.use_kv_cache:
                kv_idxs = torch.arange(x0.size(0), device=self.device).repeat_interleave(repeat_nums)
            else:
                kv_idxs = None

            if self.norm_method == 0:
                amps_value = torch.add(amps_value.repeat_interleave(repeat_nums, 0), amp_k_mask[mask_count])
            else:
                amps_value = torch.mul(amps_value.repeat_interleave(repeat_nums, 0), amp_k_mask[mask_count])
            l += interval
    ...
    def forward_sample_rank(
        ...
        if False:
            from pynqs.utils.distributed import gather_tensor

            wf_all = gather_tensor(wf, self.device)
            counts_all = gather_tensor(sample_counts, self.device)
            unique_all = gather_tensor(sample_unique, self.device)
