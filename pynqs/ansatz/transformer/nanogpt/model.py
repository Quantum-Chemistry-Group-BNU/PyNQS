...
class LayerNorm(nn.Module):
    ...
    def __init__(self, ndim, bias, dtype):
        super().__init__()
        self.weight = nn.Parameter(torch.ones(ndim, dtype=dtype))
        self.bias = nn.Parameter(torch.zeros(ndim, dtype=dtype)) if bias else None
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if not self.flash:
            if self.rank == 0:
                print("WARNING: using slow attention. Flash Attention requires PyTorch >= 2.0", flush=True)
    ...
    def forward(self, x, kv_cache=None, kv_idxs=None):
        ...
        if self.flash and not use_kv_cache:
...
class GPT(nn.Module):
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(
            dict(
                wte=nn.Embedding(
                    config.vocab_size + 1, config.n_embd
...
if __name__ == "__main__":
    ...
    x = torch.Tensor([[0, 1, 2, 3], [0, 1, 2, 3]]).type(torch.long)
    print(f"in: {x}")
