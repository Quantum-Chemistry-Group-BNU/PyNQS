...
sys.path.append("./")
from pynqs.ansatz.transformer.nanogpt.model import get_decoder_amp
from pynqs.ansatz.symmetry import symmetry_mask
from pynqs.ansatz.utils import OrbitalBlock, joint_next_samples, SoftmaxLogProbAmps
...
class MPSdecoder(nn.Module):
    ...
    def forward_psi(
        ...
        target = input.to(input.dtype)
        if get_amp:
            input = target * 2 - 1  # 0/1 model input -> legacy amp input -1/1
        if q == None:
            q = self.nqubits // 2
        if q == 0:
    ...
    def forward_psi_trans(
        ...
        for i in range(0, self.nqubits // 2):
            ...
            # breakpoint()
            if self.pmode == None:  # (n_batch, n_cond, dcut, dcut)
                wf_nn[i] = torch.einsum("ijkla->ajkli", wf_nn[i])  # (4, n_cond, dcut, dcut, n_batch)
                if i == 0:
                    wf_nn[i] = torch.einsum(
                        "aijkl,j->aikl",
                        wf_nn[i],
        ...
        for l in range(0, q):
            ...
            if l == self.nqubits // 2 - 1 and (not l == 0):
                ...
                # psi_amp_l = torch.cumprod(psi_amp_l, dim=1)
                psi_amp_l = torch.einsum("ai,ai->ia", psi_amp_l, psi_amp_l.conj()).real  # (n_batch, 4)
            else:
                if l == 0:  # (4, dcut, n_batch)
                    # psi_amp_l = torch.cumprod(psi_amp_l, dim=1)
                    psi_amp_l = torch.einsum("ajk,ajk->ka", psi_amp_l, psi_amp_l.conj()).real  # (n_batch, 4)
                else:
