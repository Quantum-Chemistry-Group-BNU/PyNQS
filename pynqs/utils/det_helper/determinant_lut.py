...
from pynqs.libs.C_extension import wavefunction_lut, packbits, unpackbits
from pynqs.utils.public_function import torch_sort_onv
...
def get_all_orb_onv(
    ...
    for k in range(0, sorb, 2):
        if k == 0:
            ...
            if _mask.sum().item() == 0:
                min_sorb_idx = k - 2
            y = packbits(unique[_mask].to(torch.uint8), unique.size(1))
            if k > 2:
                y0 = packbits(unique[_mask][:, :-2].to(torch.uint8), unique.size(1) - 2)
    ...
    y = packbits(unique.to(torch.uint8), unique.size(1))
    idx = torch_sort_onv(y)
    onv_lst[0][-1] = y[idx] # Exact optimization
    y0 = packbits(unique[:, :-2].to(torch.uint8), unique.size(1) -2)
    ...
    for i in range(len(onv_lst[0])):
        if onv_lst[0][i] is None:
            ...
            next_onv = packbits(next_states.to(torch.uint8), i_sorb)
            # breakpoint()
