...
from pynqs.utils.public_function import get_special_space
...
def read_integral(
    ...
    if external_onstate is not None:
        ...
        if given_sorb != sorb:
            ...
            fci_size = n1 * n2
            warnings.warn(f"The CI-space({onstate.size(0):.3E}) is part of the FCI-space({fci_size:.3E})")
        if save_onstate:
            if prefix is None:
                prefix = "onstate"
...
def change_integral_order(
    ...
    try:
        from pynqs.libs.C_extension import compress_h1e_h2e as compress
        from pynqs.libs.C_extension import decompress_h1e_h2e as decompress
    except ImportError:
...
def extract_kij(h1e: Tensor, h2e: Tensor, sorb: int) -> Tensor:
    """
    extract kij from compressed 'Antisymmetrize V[pqrs]=<pq||rs>'
    """
...
def spin_to_spatial(h1e_spin: Tensor, h2e_spin: Tensor, sorb: int) -> tuple[Tensor, Tensor]:
    """
    extract 'spatial (pq|rs)' from compressed 'spin Antisymmetrize V[pqrs]=<pq||rs>'
    """

    # check size, h1e and h2e is 1D.
    pair = sorb * (sorb - 1) // 2
    device = h1e_spin.device
    assert h2e_spin.size(0) == (pair * (pair + 1)) // 2
    from pynqs.libs.C_extension import decompress_h1e_h2e as decompress

    h1e_spin, h2e_spin = decompress(h1e_spin.cpu().numpy(), h2e_spin.cpu().numpy(), sorb)
    sorb = int(h1e_spin.shape[0])
    norb = sorb // 2
    alpha = 2 * np.arange(norb)
    beta = 2 * np.arange(norb) + 1
    h1e_spatial = h1e_spin[0::2, 0::2]

    # block[p,q,r,s] = <p alpha, q beta || r alpha, s beta>.
    # The exchange term vanishes for opposite spins, so block[p,q,r,s] = (pr|qs).
    # Swap axes 1 and 2 to return h2[p,q,r,s] = (pq|rs).
    block = h2e_spin[np.ix_(alpha, beta, alpha, beta)]
    h2e_spatial = block.transpose(0, 2, 1, 3)

    h1e_spatial = torch.from_numpy(h1e_spatial).to(device)
    h2e_spatial = torch.from_numpy(h2e_spatial).to(device)
    return h1e_spatial, h2e_spatial
