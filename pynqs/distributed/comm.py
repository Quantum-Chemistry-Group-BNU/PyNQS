...
def processes_synchronize() -> None:
    """
...
def all_to_all_tensor(
    ...
    if isinstance(tensor, Tensor):
        ...
        N_single = tensor.size(0)
    else:
        # reduce the memory
        tensor_list = tensor
        if world_size == 1:
            return tensor_list[0]
        min_batch = tensor_list[0].size(1)
        res = 0
        ...
        for t in tensor_list:
            assert t.dim() == 2
            assert t.size(1) == min_batch
            if not t.is_contiguous():
                raise NotImplementedError(f"tensor_list must be is contiguous")
        N_single = tensor_list[0].size(0)

    N_single = torch.tensor([N_single], device=device, dtype=torch.int64)
    N_all = torch.cat(all_gather_tensor(N_single, device=device))
    out_storage = torch.empty(N_all.sum().item(), min_batch, device=device, dtype=dtype)
    out_tensor = list(out_storage.split(list(N_all.cpu())))
