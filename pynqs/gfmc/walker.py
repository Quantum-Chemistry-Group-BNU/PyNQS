...
from pynqs import dtype_config
...
from pynqs.sample import ElocParams, Sampler, SampleParams
from pynqs.sample.exact import construct_space_lut
from pynqs.energy.flip import Func
from pynqs.ci import CIAnsatz
...
class GFMC:
    def __init__(
        ...
        max_iter: int,
        branch_interval: int,
        vmc_sample_params: SampleParams,
        p_step: int = 50,
        ...
        self.real_dtype = dtype_config.real_dtype
        self.complex_dtype = dtype_config.complex_dtype
        if not isinstance(eloc_param, ElocParams):
            raise TypeError(f"'eloc_param' must be ElocParams, got {type(eloc_param)}")
        if not isinstance(vmc_sample_params, SampleParams):
            raise TypeError(f"'vmc_sample_params' must be SampleParams, got {type(vmc_sample_params)}")
        assert vmc_sample_params.method_sample not in (
            SampleMethod.EXACT,
            "Exact",
            "EXACT",
        ), "GFMC does not support 'method_sample=EXACT'; please use AR/MCMC/PTMC/HybridMC."

        self.eloc_param = eloc_param
        self.eloc_method = self.eloc_param.method
        # assert self.eloc_method == ElocMethod.SIMPLE
        self.use_unique = self.eloc_param.use_unique
        ...
        if self.dtype.is_complex:
            raise TypeError(f"GFMC does not support complex dtype, got {self.dtype}")

        self.vmc_sampler = Sampler(
            self.nqs,
            ele_info,
            sample_params=vmc_sample_params,
            use_spin_raising=False,
            spin_raising_coeff=1.0,
            only_sample=False,
        )
        self.nqs = torch.compile(trial_wf.module, dynamic=True, fullgraph=True)
    ...
    def sample_update(
        ...
        debug_flag = False
        n_comb = comb_x.size(1)
        green_kernel = torch.nan_to_num(green_kernel, nan=0.0, posinf=0.0, neginf=0.0)
        green_kernel = torch.clamp(green_kernel, min=0.0)

        beta = green_kernel.sum(-1, keepdim=True)  # [nbatch, 1]
        if debug_flag:
            bad_beta = (~torch.isfinite(beta.reshape(-1))) | (beta.reshape(-1) <= 0)
            if bad_beta.any():
                n_bad = int(bad_beta.sum().item())
                logger.warning(
                    f"[GFMC] invalid beta rows: {n_bad}/{beta.size(0)}. "
                    f"Fallback to diagonal move on these rows."
                )
                green_kernel[bad_beta] = 0.0
                green_kernel[bad_beta, 0] = 1.0
                beta = green_kernel.sum(-1, keepdim=True)
        ...
        if debug_flag:
            out_of_bounds = (index < 0) | (index >= n_comb)
            if out_of_bounds.any():
                n_oor = int(out_of_bounds.sum().item())
                idx_max = int(index.max().item())
                logger.warning(
                    f"[GFMC] searchsorted out-of-bounds index: {n_oor}/{index.numel()}, "
                    f"max index={idx_max}, n_comb={n_comb}. Clamp to valid range."
                )
            index = index.clamp_(min=0, max=n_comb - 1)
    ...
    def batch_green_kernel(
        ...
        if ~torch.isfinite(eloc).any():
            idx = torch.nonzero(torch.isfinite(eloc), as_tuple=False)
            logger.info(f"NaN/Inf index: {idx[:10]}")
            raise FloatingPointError(f"E_loc has NaN/Inf")
    ...
    def branching(
        ...
        # simple distributed
        # x_all = gather_tensor(x, self.device)
        # w_all = gather_tensor(weight, self.device)
        # if self.rank == 0:
        ...
        # else:
        #     x_new: Tensor = None
        # x_new = scatter_tensor(x_new, self.device, torch.uint8)

        w_sum = all_gather_tensor(weight.sum(0, keepdim=True), self.device)
        w_sum_all = torch.cat(w_sum).cumsum(0)
        x_size = torch.tensor([x.size(0)], dtype=torch.int64, device=self.device)
        x_size_all = all_gather_tensor(x_size, self.device)
        ...
        x_all = gather_tensor(x, self.device)
        cum_prob_all = gather_tensor(cum_prob, self.device)
        rand_prob_all = gather_tensor(rand_prob, self.device)
        # w_all = gather_tensor(weight, self.device)
    ...
    def run(self) -> None:
        ...
        self.WF_LUT = None
        if True:
            state, state_prob, (eloc, sloc), (eloc_mean, sloc_mean) = self.vmc_sampler.run(epoch=0)
            if self.use_vmc_sample:
                # repeat_nums = (state_prob / self.world_size * self.vmc_sampler.n_sample).long()
                # nums_all = gather_tensor(repeat_nums, self.device)
                state_all = gather_tensor(state, self.device)
                if self.rank == 0:
                ...
                prob_all = gather_tensor(state_prob, self.device)
                state_all = gather_tensor(state, self.device)
                eloc_all = gather_tensor(eloc, self.device)
                sloc_all = gather_tensor(sloc, self.device)
                if self.rank == 0:
            ...
            from pynqs.utils.ci import energy_CI
        ...
        if False:
            # speed up eloc for Testing
            fp_batch = self.eloc_param.fp_batch
            file = "./molecule/Fe2S2-fci.pth"
            ci_space = torch.load(file, weights_only=False, map_location="cpu")
            ci_space = ci_space.to(self.device)
            assert self.ci_space.size(0) == self.vmc_sampler.SamplerState.fci_size
            wf_lut = construct_space_lut(
                self.nqs.module,
                ci_space,
                self.sorb,
                fp_batch,
        ...
        # n_sample = all_reduce_tensor()
        n_total = torch.tensor([nbatch], device=self.device, dtype=torch.int64)
        dist.all_reduce(n_total, dist.ReduceOp.SUM)
        n_total = n_total.item()
        prob_uniform = torch.full_like(self.weight, 1.0 / n_total, dtype=self.real_dtype)
        logger.info(f"n_total: {n_total}, prob_uniform: {prob_uniform.sum()}")
        cost_time = []
        ...
        for epoch in range(self.max_iter):
            ...
            all_reduce_tensor([_sum_w, _sum_b], dist.ReduceOp.SUM)
            stats_eloc = operator_statistics(eloc, self.weight / _sum_w, n_total, "E")
            stats_eloc_p = operator_statistics(eloc, beta_prod / _sum_b, n_total, "E-prod")
            ...
            # update sample
            _max_w = weight_new.max()
            _max_b = beta.max()
            all_reduce_tensor([_max_w, _max_b], dist.ReduceOp.MAX)
            ...
            stats_weight = operator_statistics(self.weight, prob_uniform, n_total, "ω")
            stats_beta = operator_statistics(beta_prod, prob_uniform, n_total, "β-prod")
            if self.rank == 0:
                s = str(stats_weight) + "\n"
                s += str(stats_beta)
                logger.info(s, master=True)
            ...
            if epoch > 0 and (epoch % self.branch_interval == 0):
                ...
                all_reduce_tensor([_sum_w], dist.ReduceOp.SUM, True)
