...
def jacobian_vector(module: nn.Module, states: Tensor) -> Tensor:
    ...
    for k, dws in ft_per_sample_grads.items():
        out.append(torch.cat(tuple(dws[l].reshape(1, -1).detach() for l in range(states.shape[0]))))
...
def jacobian_simple(module: nn.Module, states: Tensor) -> Tensor:
    ...
    out = states.new_empty([states.size(0), sum(map(torch.numel, params))], dtype=params[0].dtype)
