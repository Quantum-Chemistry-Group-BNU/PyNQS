...
from pynqs.optim import BaseVMCOptimizer
from pynqs.utils.public_function import (
    split_length_idx,
    split_batch_idx,
)
from pynqs.utils.det_helper import DetLUT
from pynqs.distributed import all_reduce_tensor, processes_synchronize, all_gather_tensor
from pynqs.utils.ci import CIWavefunction
from pynqs.libs.C_extension import get_hij_torch, unpackbits, packbits, get_comb_tensor
...
class NqsCi(BaseVMCOptimizer):
    ...
    def __init__(
        ...
        **vmc_opt_kwargs: dict,
        ...
        self.factory_kwargs = {"device": self.device, "dtype": self.default_dtype}
        ...
        # init total-coeff
        self.total_coeff = torch.ones(dim, dtype=self.default_dtype).to(self.device)
        ...
        if self.rank == 0:
            ...
            if self.default_dtype == torch.complex128:
    ...
    def make_ci_hij(self) -> None:
        ...
        if self.use_spin_raising:
            ...
            hij_spin = get_hij_torch(self.ci_det, self.ci_det, h1e_spin, h2e_spin, self.sorb, self.nele)
    ...
    def init_ci_nqs(self) -> list[Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        if not self.use_sample_space or self.exact:
            x1 = unpackbits(unique_comb, self.sorb)
        else:
    ...
    def _batch_forward(self, x: Tensor, MAX_DIM: int = -1) -> Tensor:
        ...
        if dim == 0:
            placeholder = torch.empty(0, dtype=self.default_dtype, device=self.device)
            return placeholder
        ...
        result = torch.empty(dim, dtype=self.default_dtype, device=self.device)
    ...
    def make_ci_nqs(self) -> None:
        ...
        if self.exact:
            ...
            psi_x1[lut_idx] = lut_value.to(self.default_dtype)
        ...
        torch.cat(all_gather_tensor(rank_value, self.device), out=all_value)
        ...
        if self.use_spin_raising:
            ...
            torch.cat(all_gather_tensor(rank_value_spin, self.device), out=all_value)
    ...
    def calculate_new_term(
        ...
        if self.grad_strategy == 2:
            ...
            x = torch.zeros(state.size(0), dtype=self.default_dtype, device=self.device)
            return x

        # Single-Rank
        bra = packbits(state.to(torch.uint8), self.sorb)
        ...
        elif self.default_dtype == torch.double:
    ...
    def new_nqs_grad(
        ...
        def batch_loss_backward(begin: int, end: int) -> None:
            ...
            loss = 2 * (c0 * scale * (torch.sum(state_prob_batch * log_psi_batch.conj() * corr))).real
            loss.backward()
    ...
    def run(self) -> None:
        ...
        for epoch in range(self.max_iter):
            t0 = time.time_ns()
            state, state_prob, (eloc, sloc), (eloc_mean, sloc_mean) = self.sampler.run(epoch=epoch)
    ...
    def operator_expected(
        ...
        if len(self.e_lst) == 0:  # not CI-NQS iteration
            ...
            state, state_prob, (eloc, sloc), (eloc_mean, sloc_mean) = self.sampler.run(epoch=self.max_iter)
            sample_state = unpackbits(state, self.sorb)
            if self.exact:
                with torch.no_grad():
                    phi_nqs = self.model(sample_state)
        ...
        # Run sampling
        state, state_prob, (eloc, sloc), (eloc_mean, sloc_mean) = self.sampler.run(self.max_iter)
        sample_state = unpackbits(state, self.sorb)
        ...
        if self.exact:
            ...
            if WF_LUT is None:
                raise ValueError(f"Use LUT to speed up <phi_nqs|H|phi_nqs>")
            # onv = packbits(state.to(torch.uint8), self.sorb)
            not_idx, phi_nqs = WF_LUT.lookup(state)[1:]
