...
from pynqs.sample import Sampler
...
from pynqs.utils.ci import CIWavefunction, energy_CI
from pynqs.libs.C_extension import unpackbits
from pynqs.config import dtype_config


# TODO:(zbwu-25-11-07) check MultiProcessing using DDP
class CITrain:
    """
    ...
    def __init__(
        ...
        self.ci_state = unpackbits(self.onstate, self.sorb)
    ...
    def train(
        ...
        if flag_energy:
            ...
            if dim > 10000:
                ...
                warnings.warn(f"CI-coeff dim: {dim} to large, does not calculate e-ref avoiding OOM")
                self.eCI_ref = 0.00
            else:
        ...
        for epoch in range(self.pre_max_iter + 1):
            ...
            reduce_loss = all_reduce_tensor(loss, in_place=False)[0]
            reduce_ovlp = all_reduce_tensor(ovlp, in_place=False)[0]
            processes_synchronize()
            if self.rank == 0:
            ...
            if (epoch == 0) or (epoch == self.pre_max_iter):
                if flag_energy:
                    e_total = self.sampler.run(0)[3][0] + self.sampler.ecore
                    if epoch == 0:
                        eCI_begin = e_total
                    else:
                        eCI_end = e_total
        ...
        # p = self.sampler.run(epoch=0)
        # fci_space = self.sampler.ci_space
        # e_ci1 = get_energy(self.model((unpackbits(fci_space, self.sorb))), fci_space)
        ...
        if self.rank == 0:
            ...
            if flag_energy:
                s += f"Energy ref, begin and end : {self.eCI_ref:.8f} {eCI_begin:.8f}, {eCI_end:.8f}"
            logger.info(s, master=True)
    ...
    def clip_grad(self, epoch: int) -> None:
        ...
        if self.use_clip_grad and epoch > self.start_clip_grad:
            x = nn.utils.clip_grad_norm_(self.model.parameters(), max_norm=self.max_grad_norm, foreach=True)
            if self.rank == 0:
    ...
    def QGT_loss(
        ...
        with torch.no_grad():
            ...
            if self.exact:
                ...
                sample_unique = scatter_tensor(fci_space, self.device, torch.uint8)
                sample_counts = torch.ones(sample_unique.size(0), dtype=torch.int64, device=self.device)
                processes_synchronize()
                # sample_unique = self.sampler.ele_info.ci_space.clone()
            else:
                sample_unique, sample_counts, state_prob = self.sampler.sampling(epoch=epoch)

        states_sample = unpackbits(sample_unique, self.sorb)
        if self.exact:
            # Gather all psi from very rank
            with torch.no_grad():
                psi_sample = self.model(states_sample).to(self.dtype)
            all_psi = gather_tensor(psi_sample, self.device)
            processes_synchronize()
            if self.rank == 0:
            ...
            state_prob = scatter_tensor(all_prob, self.device, self.dtype)
        ...
        ovlp_mean = all_reduce_tensor(ovlp, in_place=False)[0]
    ...
    def sample_ovlp_grad(
        ...
        state_prob = state_prob * self.world_size

        def batch_loss_backward(begin: int, end: int) -> None:
            nonlocal loss_sum
            log_psi_batch = call_abs(self.model(states[begin:end].requires_grad_())).to(dtype).log()
            # log_psi_batch = self.model(states[begin:end].requires_grad_()).to(dtype).log()
    ...
    def _get_rank_psi0(self, MAX_FP_DIM: int = -1) -> Tensor:
        ...
        psi0 = torch.dot(self.rank_pre_ci_coeff.conj(), psi_rank)
        # all-rank
        all_reduce_tensor(psi0)
        processes_synchronize()
    ...
    def test_ovlp_grad(
        ...
        def batch_loss_backward(begin: int, end: int) -> None:
            ...
            loss = (psi_batch - ci_coeff_batch).norm().pow(2)
            loss.backward()
    ...
    def plot_figure(self, prefix: str = None) -> None:
        ...
        np.savez(prefix + "-pre-train", np.asarray(self.loss_list), np.asarray(self.ovlp_list), param_L2)
